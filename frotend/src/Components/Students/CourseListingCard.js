...
const CourseListingCard = ({ courses, mycourses }) => {
  return (
    <>
    {/* <pre>{JSON.stringify(courses, null, 4 )}</pre> */}
    {
      ...
      <div className="col-md-6 col-sm-12 col-lg-3 d-flex justify-content-center">
        <LinkContainer to={mycourses ? `/start-course/${course.slug}` : `/course-details/${course.slug}`}>
        <div className="card shadow mb-5 bg-white rounded">
          ...
          <div className="card-body">
              ...
              <span className='proCardContent' style={{ fontWeight : "bold"}}><i className="fa-solid fa-align-justify"></i> {course.lessons && course.lessons.length} Lessons</span>
              {
                !mycourses && 
                <>
                  <span className='realPrice' style={{ float : "right"}}><s>{course.paid && "₹ " + course.price}</s></span> <br />
                </>
              }
            <div className="mt-3">
                ...
                {
                  mycourses ? <span style={{ float : "right"}} className='btn btn-success'>Start Course</span> :
